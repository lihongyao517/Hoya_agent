...
describe("layout transition", () => {
  test("blank profiles default to the classic layout", () => {
  ...
  test("sunset replaces the toggle with a dismissible notice", () => {
    ...
    expect(resolveNewLayoutDesigns(true, undefined)).toBe(false)
  })
  ...
  test("does not auto-enable the new layout on upgrade", () => {
    expect(shouldEnableNewLayout("v1.17.19", "1.17.20")).toBe(false)
    expect(shouldEnableNewLayout("1.16.9", "2.0.0")).toBe(false)
  })

  test("does not enable the new layout when no previous version was recorded", () => {
