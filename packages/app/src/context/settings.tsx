...
// HoyaAgent defaults to the classic (old) layout. New layout remains available via settings when eligible.
const legacyNewLayoutDesignsDefault = false
export const newLayoutDesignsDefault = false
// Existing users can switch layouts until local midnight on this date. Set new Date(YYYY, M-1, D) to show.
// Keep far in the future so the old layout is not auto-retired.
...
export function shouldEnableNewLayout(_previous: string | undefined, _current: string | undefined) {
  // HoyaAgent keeps the classic layout by default; never auto-migrate users onto the new layout.
  return false
}
...
export function resolveNewLayoutDesigns(_retired: boolean, preference: boolean | undefined, fallback = false) {
  // HoyaAgent never forces the new layout after sunset; honor preference or classic default.
...
export const { use: useSettings, provider: SettingsProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const layoutTransition = createMemo(() =>
      // Always allow switching layouts in HoyaAgent; default remains classic.
