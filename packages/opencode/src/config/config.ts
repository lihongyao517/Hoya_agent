...
function globalConfigFile() {
  const candidates = ["hoya.jsonc", "hoya.json", "opencode.jsonc", "opencode.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "hoya.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "hoya.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("hoya", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".hoya") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["hoya.json", "hoya.jsonc", "opencode.json", "opencode.jsonc"]) {
          ...
          // Auto-discovered plugins under `.hoya/plugin(s)` are already local files, so ConfigPlugin.load
