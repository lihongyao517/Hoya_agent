...
import { getMaterializedBranchPathOrCompaction } from "./branch-entries.js";
import { decodeEntry, encodeEntry } from "./session-entries.js";
import { applyEntryToMaterializedState, createEmptyMaterializedState, entryMaterializedValues, materializedStateFromRows, materializedStateValues, serializeSummary, sessionStatsFromMaterializedState, } from "./session-materialized.js";
import { advanceSequence, getNextSequence } from "./session-sequences.js";
import { rowToMetadata } from "./sessions.js";
import { generateEntryId, invalidSession, leafIdAfterEntry } from "./shared.js";
async function decodeEntryRows(entryRows) {
    const entries = [];
    let leafId = null;
...
async function loadEntryRowsByIds(db, sessionId, entryIds) {
...
async function loadActiveBranchId(db, sessionId) {
    ...
    const row = await db
        .prepare("SELECT branch_id FROM branch_entries WHERE session_id = ? ORDER BY entry_seq DESC, branch_id DESC LIMIT 1")
        .get(sessionId);
    return row?.branch_id ?? null;
}
async function hasExistingChild(db, sessionId, parentId) {
...
async function loadSqliteStorage(db, sessionId) {
    const row = await db
    ...
    const materializedRow = await db
        ...
        .get(sessionId);
    if (!materializedRow)
        throw invalidSession(`missing materialized row for session ${sessionId}`);
    const entryMaterializedRows = await db
        .prepare("SELECT session_id, entry_seq, type, payload FROM entry_materialized WHERE session_id = ? ORDER BY entry_seq, type")
        .all(sessionId);
    return {
        row,
        leafId,
...
export class SqliteSessionStorage {
    ...
    activeBranchId;
    materializedState;
    async getPathToRootOrCompactionEntries(leafId) {
        if (leafId === null)
            return [];
        const path = [];
    ...
    async materializeBranch(leafId) {
        ...
        const entryRowsById = await loadEntryRowsByIds(this.db, this.metadata.id, path.map((entry) => entry.id));
    ...
    async appendToActiveBranch(entryId, parentId) {
    ...
    constructor(db, metadata, entries, leafId, activeBranchId, materializedState) {
        this.db = db;
    ...
    static async open(db, metadata) {
        const loaded = await loadSqliteStorage(db, metadata.id);
        return new SqliteSessionStorage(db, rowToMetadata(loaded.row, metadata.path), null, loaded.leafId, loaded.activeBranchId, loaded.materializedState);
    }
    static async create(db, path, options) {
        const createdAt = new Date().toISOString();
        await db
            .prepare("INSERT INTO sessions (id, created_at, metadata, cwd, parent_session_id, active_leaf_id) VALUES (?, ?, ?, ?, ?, ?)")
            .run(options.sessionId, createdAt, options.metadata === undefined ? null : JSON.stringify(options.metadata), options.cwd, options.parentSessionId ?? null, null);
        ...
        }, [], null, null, createEmptyMaterializedState());
    }
    async getMetadata() {
    ...
    async appendEntry(entry) {
        ...
        const previousMaterializedState = {
        ...
        try {
            ...
            await this.db.transaction(async () => {
                ...
                await this.db
                    .prepare("INSERT INTO session_entries (session_id, id, entry_seq, parent_id, type, timestamp, payload) VALUES (?, ?, ?, ?, ?, ?, ?)")
    ...
    async getEntry(id) {
        ...
        const row = await this.db
            .prepare("SELECT session_id, id, entry_seq, parent_id, type, timestamp, payload FROM session_entries WHERE session_id = ? AND id = ?")
            .get(this.metadata.id, id);
        if (!row)
            return undefined;
        try {
    ...
    async findEntries(type) {
        const rows = await this.db
            .prepare("SELECT session_id, id, entry_seq, parent_id, type, timestamp, payload FROM session_entries WHERE session_id = ? AND type = ? ORDER BY entry_seq")
            .all(this.metadata.id, type);
        const entries = [];
    ...
    async getEntries(options) {
        ...
        if (limit !== undefined) {
            const beforeOrAtEntrySeq = options?.afterEntrySeq ??
                (await this.db
                    .prepare("SELECT entry_seq FROM session_entries WHERE session_id = ? ORDER BY entry_seq DESC LIMIT 1")
                    .get(this.metadata.id))?.entry_seq;
            if (beforeOrAtEntrySeq === undefined) {
                return [];
            }
            const rows = await this.db
                .prepare("SELECT session_id, id, entry_seq, parent_id, type, timestamp, payload FROM session_entries WHERE session_id = ? AND entry_seq <= ? ORDER BY entry_seq DESC LIMIT ?")
                .all(this.metadata.id, beforeOrAtEntrySeq, limit);
        ...
        const rows = await this.db
            .prepare("SELECT session_id, id, entry_seq, parent_id, type, timestamp, payload FROM session_entries WHERE session_id = ? ORDER BY entry_seq")
            .all(this.metadata.id);
        const entries = (await decodeEntryRows(rows)).entries;
        for (const entry of entries) {
            this.byId.set(entry.id, entry);
        }
