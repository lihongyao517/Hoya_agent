import { invalidSession, isRecord } from "./shared.js";
function compareModelThinkingConfig(left, right) {
    return (left.provider.localeCompare(right.provider) ||
        left.modelId.localeCompare(right.modelId) ||
        left.thinkingLevel.localeCompare(right.thinkingLevel));
}
function normalizeModelThinkingConfigs(configs) {
...
function addModelThinkingConfig(state, provider, modelId, thinkingLevel) {
...
export function createEmptyMaterializedState() {
    return {
        name: undefined,
        messageCount: 0,
        cachedTokens: 0,
...
export function applyEntryToMaterializedState(state, entry) {
    switch (entry.type) {
        case "session_info":
...
function parseSummary(json) {
    ...
    try {
        ...
        throw invalidSession(`materialized session summary is not valid JSON`, error instanceof Error ? error : undefined);
    }
    ...
    return {
        ...
        currentModel: currentModel && isRecord(currentModel)
            ? { provider: currentModel.provider, modelId: currentModel.modelId }
            : (currentModel ?? undefined),
        currentThinkingLevel: currentThinkingLevel ?? undefined,
    };
}
function parseEntryMaterializedPayload(row) {
    try {
        return JSON.parse(row.payload);
    }
    catch (error) {
        throw invalidSession(`materialized entry row ${row.entry_seq} is not valid JSON`, error instanceof Error ? error : undefined);
    }
}
export function materializedStateFromRows(summaryRow, entryRows) {
...
export function sessionStatsFromMaterializedState(state) {
    return {
        messageCount: state.messageCount,
        cachedTokens: state.cachedTokens,
        uncachedTokens: state.uncachedTokens,
...
export function materializedStateValues(sessionId, state) {
    return [sessionId, serializeSummary(state)];
}
export function entryMaterializedValues(entry) {
    switch (entry.type) {
        case "label":
            return [
                {
...
//# sourceMappingURL=session-materialized.js.map