...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["hoya.json", "hoya.jsonc", "opencode.json", "opencode.jsonc"]
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => path.basename(item) === ".hoya")
        .toReversed()
        .map((directory) => AbsolutePath.make(directory)),
    ]
    // A config closer to the opened directory should win over one higher up.
    // Search starts nearby, so reverse the results before applying them.
    const directPaths = discovered.filter((item) => path.basename(item) !== ".hoya").toReversed()
    ...
    // global config, project files, then `.hoya` files.
