...
const { checkForUpdates, compareVersions, releaseTagFromUrl, versionParts } = require('../electron/update-service.cjs')
...
test('compares GitHub tags against the desktop version', () => {
  ...
  assert.equal(releaseTagFromUrl('https://github.com/lihongyao517/Hoya_agent/releases/tag/v1.2.1'), 'v1.2.1')
  assert.equal(releaseTagFromUrl('https://github.com/lihongyao517/Hoya_agent/releases'), '')
})

test('returns update metadata from the latest published GitHub release', async () => {
  const result = await checkForUpdates('0.1.3', async () => ({
    ok: true,
    url: 'https://github.com/lihongyao517/Hoya_agent/releases/tag/v1.1.0',
  }))

  assert.equal(result.currentVersion, '0.1.3')
  assert.equal(result.latestVersion, 'v1.1.0')
